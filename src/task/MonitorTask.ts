...
import { ISdkBitrateFrame, SdkNotificationFrame, SdkSignalFrame } from '../signalingprotocol/SignalingProtocol';
    ...
    if (event.type === SignalingClientEventType.ReceivedSignalFrame) {
      if (event.message.type === SdkSignalFrame.Type.NOTIFICATION) {
        switch (event.message.notification.level) {
          ...
          default:
            this.logger.error(
              `Received notification from server with unknown level ${
                event.message.notification.level
